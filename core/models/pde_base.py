...
class PDE_NN_BASE(nn.Module):
    ...
    def reset_aux_head(self):
        if self.aux_pace_model:
            self.aux_pace_model.reset_head()
        else:
            raise NotImplementedError

    def set_aux_head_linear_probing_mode(self, mode: bool=True):
        # self.linear_probing_mode = mode
        if self.aux_pace_model:
            self.aux_pace_model.set_linear_probing_mode(mode)
        else:
            raise NotImplementedError
    ...
    def set_aux_head_mode(self, mode: bool = True) -> None:
    ...
    def disable_aux_head_params(self):
        if self.aux_pace_model:
    ...
    def enable_aux_head_params(self):
        if self.aux_pace_model:
            for param in self.aux_pace_model.parameters():
                param.requires_grad = True
        if self.aug_feature is not None:
    ...
    def enable_head_params(self):
        for param in self.head.parameters():
            param.requires_grad = True
        if self.aux_pace_model:
            for param in self.aux_pace_model.head.parameters():
                param.requires_grad = True